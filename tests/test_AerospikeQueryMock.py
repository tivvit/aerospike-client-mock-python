...
class TestAerospikeQueryMock(unittest.TestCase):
    ...
    def test_query(self):
        ...
        query.select('a', 'c')
        self.assertEqual([(('a', 'b', 3), {'a': 3, 'c': None}, {'gen': 1, 'ttl': 0}), (('a', 'b', 2), {'a': 2, 'c': None}, {'gen': 1, 'ttl': 0}), (('a', 'b', 1), {'a': 1, 'c': None}, {'gen': 1, 'ttl': 0})], query.results())

    def test_query_namespace(self):
        query = self.asm.query('a')
        query.select('a', 'b')
        self.assertEqual([(('a', 'b', 3), {'a': 3, 'b': 3}, {'gen': 1, 'ttl': 0}),
            (('a', 'b', 2), {'a': 2, 'b': 2}, {'gen': 1, 'ttl': 0}),
            (('a', 'c', 4), {'a': 4, 'b': 4}, {'gen': 1, 'ttl': 0}),
            (('a', 'b', 1), {'a': 1, 'b': 1}, {'gen': 1, 'ttl': 0})], query.results())
    ...
    def test_query_callback(self):
        ...
        self.assertEqual([((('a', 'b', 3), {'a': 3, 'c': None}, {'gen': 1, 'ttl': 0}), (('a', 'b', 2), {'a': 2, 'c': None}, {'gen': 1, 'ttl': 0}), (('a', 'b', 1), {'a': 1, 'c': None}, {'gen': 1, 'ttl': 0}))], result)

    def test_query_equals(self):
        query = self.asm.query('a', 'b')
        query.select('a', 'c')
        query.where(AerospikePredicatesMock().equals("a", 1))
        self.assertEqual([(('a', 'b', 1), {'a': 1, 'c': None}, {'gen': 1, 'ttl': 0})], query.results())

    def test_query_between(self):
        query = self.asm.query('a', 'b')
        query.select('a', 'c')
        query.where(AerospikePredicatesMock().between("a", 1, 4))
        self.assertEqual([(('a', 'b', 3), {'a': 3, 'c': None}, {'gen': 1, 'ttl': 0}),
                          (('a', 'b', 2), {'a': 2, 'c': None}, {'gen': 1, 'ttl': 0})], query.results())
    ...
    def test_query_contains(self):
        ...
        self.assertEqual([(('a', 'l', 1), {'list': [1, 2, 3, 4, 5]}, {'gen': 1, 'ttl': 0})], query.results())
